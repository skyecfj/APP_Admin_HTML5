+function(a) {
	...
	b.DEFAULTS = {
		...
		var c = "disabled", d = this.$element, e = d.is("input")
				? "val"
				: "html", f = d.data();
		b += "Text", f.resetText || d.data("resetText", d[e]()), d[e](f[b]
				|| this.options[b]), setTimeout(a.proxy(function() {
							"loadingText" == b ? (this.isLoading = !0, d
									.addClass(c).attr(c, c)) : this.isLoading
		...
		return this.$element.trigger(j), j.isDefaultPrevented()
				...
				: (this.sliding = !0, f && this.pause(), this.$indicators.length
						&& (this.$indicators.find(".active")
								.removeClass("active"), this.$element.one(
						...
						? (e.addClass(b), e[0].offsetWidth, d.addClass(g), e
								.addClass(g), d.one(a.support.transition.end,
								function() {
								...
								}).emulateTransitionEnd(1e3
								* d.css("transition-duration").slice(0, -1)))
			...
			if (this.$element.trigger(b), !b.isDefaultPrevented()) {
				...
				var f = function() {
					this.$element.removeClass("collapsing")
							.addClass("collapse in")[e]("auto"), this.transitioning = 0, this.$element
	...
	a.fn.collapse = function(c) {
		return this.each(function() {
			...
			!e && f.toggle && "show" == c && (c = !c), e
					|| d.data("bs.collapse", e = new b(this, f)), "string" == typeof c
					&& e[c]()
		})
	...
	function b(b) {
		a(d).remove(), a(e).each(function() {
			...
			d.hasClass("open")
					&& (d.trigger(b = a.Event("hide.bs.dropdown", e)), b
							.isDefaultPrevented()
	...
	f.prototype.toggle = function(d) {
		...
		if (!e.is(".disabled, :disabled")) {
			...
			if (b(), !g) {
				...
				if (f.trigger(d = a.Event("show.bs.dropdown", h)), d
						.isDefaultPrevented())
					return;
				f.toggleClass("open").trigger("shown.bs.dropdown", h), e
						.focus()
			}
			return !1
		}
		...
		if (/(38|40|27)/.test(b.keyCode)) {
			...
			if (b.preventDefault(), b.stopPropagation(), !d
				...
				var h = " li:not(.divider):visible a", i = f.find("[role=menu]"
						+ h + ", [role=listbox]" + h);
				if (i.length) {
					var j = i.index(i.filter(":focus"));
					38 == b.keyCode && j > 0 && j--, 40 == b.keyCode
	...
	a.fn.dropdown = function(b) {
		...
		return a.fn.dropdown = g, this
			...
			e + ", [role=menu], [role=listbox]", f.prototype.keydown)
}(jQuery), +function(a) {
	"use strict";
	var b = function(b, c) {
	...
	b.DEFAULTS = {
				...
				|| (this.isShown = !0, this.escape(), this.$element.on(
						...
						function() {
							...
							c.$element.parent().length
									...
									.show().scrollTop(0), d
									&& c.$element[0].offsetWidth, c.$element
		...
		b && b.preventDefault(), b = a.Event("hide.bs.modal"), this.$element
				...
				&& (this.isShown = !1, this.escape(), a(document)
						...
						.off("click.dismiss.bs.modal"), a.support.transition
	...
	a.fn.modal = function(c, d) {
		...
		return a.fn.modal = c, this
			...
			function(b) {
				...
				c.is("a") && b.preventDefault(), e.modal(f, this).one("hide",
						function() {
	...
	b.DEFAULTS = {
		...
		if (this.hasContent() && this.enabled) {
			...
			var e = "function" == typeof this.options.placement
					...
					: this.options.placement, f = /\s?auto?\s?/i, g = f.test(e);
			g && (e = e.replace(f, "") || "top"), d.detach().css({
						top : 0,
						left : 0,
						display : "block"
					}).addClass(e), this.options.container ? d
					.appendTo(this.options.container) : d
			...
			if (g) {
				var k = this.$element.parent(), l = e, m = document.documentElement.scrollTop
						|| document.body.scrollTop, n = "body" == this.options.container
						? window.innerWidth
						: k.outerWidth(), o = "body" == this.options.container
						? window.innerHeight
						: k.outerHeight(), p = "body" == this.options.container
						? 0
						: k.offset().left;
			...
			a.support.transition && this.$tip.hasClass("fade")
					? d.one(a.support.transition.end, r)
							.emulateTransitionEnd(150)
					: r()
		}
	}, b.prototype.applyPlacement = function(b, c) {
		var d, e = this.tip(), f = e[0].offsetWidth, g = e[0].offsetHeight, h = parseInt(
				e.css("margin-top"), 10), i = parseInt(e.css("margin-left"), 10);
		...
		if ("top" == c && k != g && (d = !0, b.top = b.top + g - k), /bottom|top/
				.test(c)) {
			var l = 0;
			b.left < 0
					&& (l = -2 * b.left, b.left = 0, e.offset(b), j = e[0].offsetWidth, k = e[0].offsetHeight), this
					.replaceArrow(l - f + j, j, "left.html")
		} else
		...
		function b() {
			"in" != c.hoverState && d.detach(), c.$element.trigger("hidden.bs."
					+ c.type)
		}
		...
		return this.$element.trigger(e), e.isDefaultPrevented()
				...
				: (d.removeClass("in"), a.support.transition
						...
						? d.one(a.support.transition.end, b)
								.emulateTransitionEnd(150)
		...
		clearTimeout(this.timeout), this.hide().$element.off("." + this.type)
	...
	function b(c, d) {
		...
		this.$element = a(a(c).is("body") ? window : c), this.$body = a("body"), this.$scrollElement = this.$element
	...
	b.DEFAULTS = {
		...
		this.offsets = a([]), this.targets = a([]);
		{
			...
			this.$body.find(this.selector).map(function() {
				var d = a(this), e = d.data("target") || d.attr("href"), f = /^#./
						.test(e)
						&& a(e);
				return f
						&& f.length
		...
		this.activeTarget = b, a(this.selector).parentsUntil(
				this.options.target, ".active").removeClass("active");
	...
	var b = function(c, d) {
		this.options = a.extend({}, b.DEFAULTS, d), this.$window = a(window)
				.on("scroll.bs.affix.data-api",
						...
						a.proxy(this.checkPositionWithEventLoop, this)), this.$element = a(c), this.affixed = this.unpin = this.pinnedOffset = null, this
				.checkPosition()
	};
	b.RESET = "affix affix-top affix-bottom", b.DEFAULTS = {
		...
		if (this.$element.is(":visible")) {
			...
			"top" == this.affixed && (e.top += d), "object" != typeof f
					&& (h = g = f), "function" == typeof g
					&& (g = f.top(this.$element)), "function" == typeof h
			...
			if (this.affixed !== i) {
				...
				var j = "affix" + (i ? "-" + i : ""), k = a.Event(j
						+ ".bs.affix");
				this.$element.trigger(k), k.isDefaultPrevented()
						|| (this.affixed = i, this.unpin = "bottom" == i ? this
								.getPinnedOffset() : null, this.$element
								.removeClass(b.RESET).addClass(j).trigger(a
										.Event(j.replace("affix", "affixed"))), "bottom" == i
								&& this.$element.offset({
											top : c - h
													- this.$element.height()
										}))

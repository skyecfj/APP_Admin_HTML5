...
(function(global, setTimeout) {
	var req, s, head, baseElement, dataMain, src, interactiveScript, currentlyAddingScript, mainScript, subPath, version = '2.3.2', commentRegExp = /\/\*[\s\S]*?\*\/|([^:"'=]|^)\/\/.*$/mg, cjsRequireRegExp = /[^.]\s*require\s*\(\s*["']([^'"\s]+)["']\s*\)/g, jsSuffixRegExp = /\.js$/, currDirRegExp = /^\.\//, op = Object.prototype, ostring = op.toString, hasOwn = op.hasOwnProperty, isBrowser = !!(typeof window !== 'undefined'
	...
	function newContext(contextName) {
		...
		function makeModuleMap(name, parentModuleMap, isNormalized, applyMap) {
			var url, pluginModule, suffix, nameParts, prefix = null, parentName = parentModuleMap
					? parentModuleMap.name
		...
		function onError(err, errback) {
			...
			if (errback) {
				...
				each(ids, function(id) {
							...
							if (mod) {
								// Set error on module, so it skips timeout
								// checks.
								mod.error = err;
								if (mod.events.error) {
									notified = true;
		...
		function checkLoaded() {
			var err, usingPathFallback, waitInterval = config.waitSeconds
					* 1000,
			...
			eachProp(enabledRegistry, function(mod) {
						...
						if (!mod.error) {
							...
							} else if (!mod.inited && mod.fetched
									&& map.isDefine) {
								stillLoading = true;
								if (!map.prefix) {
			...
			callPlugin : function() {
				...
				on(pluginMap, 'defined', bind(this, function(plugin) {
					var load, normalizedMap, normalizedMod, bundleId = getOwn(
							bundlesMap, this.map.id), name = this.map.name, parentName = this.map.parentMap
							? this.map.parentMap.name
							: null, localRequire = context.makeRequire(
							map.parentMap, {
								enableBuildCallback : true
							});
				...
				each(this.depMaps, bind(this, function(depMap, i) {
							...
							if (typeof depMap === 'string') {
								...
								if (this.errback) {
									...
									// No direct errback on this module, but
									// something
									// else is listening for errors, so be sure
									// to
							...
							// Skip special modules like 'require', 'exports',
							// 'module'
		...
		context = {
			...
			configure : function(cfg) {
				...
				eachProp(registry, function(mod, id) {
							// If module already has init called, since it is
							// too
			...
			makeRequire : function(relMap, options) {
				...
				function localRequire(deps, callback, errback) {
					...
					context.nextTick(function() {
								...
								// Store if map config should be applied to this
								// require
				...
				mixin(localRequire, {
					...
					toUrl : function(moduleNamePlusExt) {
						var ext, index = moduleNamePlusExt.lastIndexOf('.'), segment = moduleNamePlusExt
	...
	if (isBrowser && !cfg.skipDataMain) {
		...
		eachReverse(scripts(), function(script) {
					...
					// Look for a data-main attribute to set main script for the
					// page
					// to load. If it is there, the path to data main becomes
					// the
					...
					if (dataMain) {
						...
						// Set final baseUrl if there is not already an explicit
						// one,
						// but only do so if the data-main value is not a loader
						// plugin
						// module ID.
						if (!cfg.baseUrl && mainScript.indexOf('!') === -1) {
							// Pull off the directory of data-main for use as
							// the
							// baseUrl.
